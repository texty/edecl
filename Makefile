...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "edecl", "tests")); system.time(test_check("edecl", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran edecl_$(VERSION).tar.gz
	rm -rf edecl.Rcheck/
